...
# This is an Inkscape extension to output paths to OpenSCAD polygons
...
# 9 July 2015
# Forked by Nick Robinson, to produce polygons, not extruded polygons.
#   It is nicer to add the extrusion in your project's scad file, rather than in a file generated
#   by this script. In this way, you can play with the polygon in different ways, such as a
#   twisted extrusion.
...
class OpenSCAD(inkex.Effect):
    def __init__(self):
        ...
        self.cx = 0; #float(DEFAULT_WIDTH) / 2.0
        self.cy = 0; #float(DEFAULT_HEIGHT) / 2.0
    ...
    # Convert numbers to strings in a human-pleasing way
    # "--precision" command line parameter for the max number of decimal places.
    # However, do not include decimal places, if it is a whole number.
    def convertNumber( self, n ) :
        return re.sub( '\.0*$', '', '%.*f' % (int(self.options.precision), n) )

    # Converts inkscape's points based numbers to millimeters.
    # Outputs a point in the form "[x,y]" where x and y are nicely formatted.
    def convertPoint( self, x, y ) :
        return '[%s,%s]' % (
            self.convertNumber( x * SCALE ),
            self.convertNumber( y * SCALE ) )

    def isClockwise( self, path  ) :
        total = 0;
        for i in range(0,len(path)) :
            point = path[i]
            prevPoint = path[i - 1]
            total += (point[0]-prevPoint[0]) * (point[1]+prevPoint[1])
  
        return total > 0

    def veryClose( self, a, b ) :
        l2 = (a[0] - b[0]) * (a[0] - b[0]) + (a[1] - b[1]) * (a[1] - b[1])
        # self.f.write( "l2 = " + str(l2 ) + "\n" );
        return l2 < 0.01


    # Defines a module for a 2D object of the given path.
    def convertPath(self, node):
        ...
        # Make a well formed "id" from the inkscape path's name, stripping out non-alphnumeric characters.
        id = node.get('id', '')
        if (id is None) or (id == ''):
        ...
        maxX = minX;
        maxY = minY;
        # Find the size and position of the path
        for i in range(0,len(path)) :
            for j in range( 0,len(path[i][0]) ) :
                point = path[i][0][j]
        ...
        # Remove any points which are very very close
        for i in range( 0,len(path) ) :
            for j in range(len(path[i][0])-1,-1,-1) :
                a = path[i][0][j-1]
                b = path[i][0][j]
                # inkex.errormsg( "Comparing : " + str( (a[0] - minX) * SCALE ) + " vs " + str( (b[0] - minX) * SCALE ) );
                if self.veryClose( a, b ) :
                    del( path[i][0][j-1] )

        # calculate the start index into the openscad "points" array for each path.
        index = 0;
        pathStartIndex = []
        for i in range( 0,len(path) ) :
            pathStartIndex.append(index);
            index += len( path[i][0] );
        ...
        for i in range(0,len(path)) :
            ...
            for j in range( 0,len(path[i][0]) ) :
                point = path[i][0][j]
                # Note that svg's y axis is opposite to openSCAD's, so reverse the Y values.
                self.f.write( self.convertPoint( point[0] - minX, maxY - point[1] ) )
                if j < len(path[i][0]) -1 :
                    self.f.write( ', ' )
        self.f.write(' ],')

        # Generate the "paths" array
        self.f.write( '\n    [ ' )
        for i in range(0,len(path)) :
            if i != 0 :
                self.f.write( ", " )
            ...
            # We want points to be listed in a clockwise direction. Do we need to reverse the order?
            reverse = not self.isClockwise( subpath )

            # If this subpath is a HOLE, then we actually want to list them anti-clockwise.
            if len(contained_by[i]) % 2 == 1 :
                reverse = not reverse

            for j in range(0,len(subpath)) :

                index = pathStartIndex[i] + ( len(subpath) - j - 1 if  reverse else j )
        ...
        # Finish the data
        self.f.write( ',\n    ' + self.convertPoint( maxX-minX, maxY-minY ) )
        self.f.write( ',\n    ' + self.convertPoint( minX, -maxY ) )
        self.f.write( '\n];\n\n' )
    ...
    def effect(self):
        ...
        try :
            if '/' == os.sep:
                ...
                self.f = open(
                    os.path.expanduser( self.options.fname).replace('/', os.sep), 'w' )
        except:
            inkex.errormsg('Unable to open the file ' + self.options.fname)

        try :
            self.f.write('''// Generated by "ink2scad" inkscape extension. http://nickthecoder.co.uk/software/view/ink2scad
//
...
''');
            ...
            inkex.errormsg('Unable to generate code.')
